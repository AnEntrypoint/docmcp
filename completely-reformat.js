...
async function completelyReformatDocument() {
    try {
        const auth = await getAuth();
        const docs = google.docs({ version: 'v1', auth });
        const docId = '1mWquQaVGDpf6VSiyzT4-Lxr0obslFQY-zmZnijx1KCc';
        
        // New clean content without any markdown symbols
...
4. Periodically check for new promotional emails`;
        ...
        // Get current document structure to determine range for deletion
        ...
        if (contentLength > 0 && doc.data.body.content[0].paragraph && doc.data.body.content[0].paragraph.elements.length > 0) {
            deleteEnd = doc.data.body.content[0].paragraph.elements[0].endIndex || 10000;
        }
        
        // Clear entire document and write clean content
        const requests = [
            {
                deleteContentRange: {
                    range: {
                        startIndex: 1,
                        endIndex: deleteEnd > 5000 ? 10000 : deleteEnd
                    }
                }
            },
            {
        ...
        // Apply proper formatting without any markdown
        const formatRequests = [
            ...
            // Section headings
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('Summary') + 1,
                        endIndex: cleanContent.indexOf('Summary') + 'Summary'.length + 1
                    },
                    textStyle: {
                        bold: true,
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('Promotional Emails Details') + 1,
                        endIndex: cleanContent.indexOf('Promotional Emails Details') + 'Promotional Emails Details'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 18, unit: 'PT' },
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('Analysis') + 1,
                        endIndex: cleanContent.indexOf('Analysis') + 'Analysis'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 18, unit: 'PT' },
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('Recommendations') + 1,
                        endIndex: cleanContent.indexOf('Recommendations') + 'Recommendations'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 18, unit: 'PT' },
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('Next Steps') + 1,
                        endIndex: cleanContent.indexOf('Next Steps') + 'Next Steps'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 18, unit: 'PT' },
            ...
            // Email headings
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('1. Pinnacle Promotion') + 1,
                        endIndex: cleanContent.indexOf('1. Pinnacle Promotion') + '1. Pinnacle Promotion'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 14, unit: 'PT' },
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('2. AutoTrader Newsletter') + 1,
                        endIndex: cleanContent.indexOf('2. AutoTrader Newsletter') + '2. AutoTrader Newsletter'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 14, unit: 'PT' },
            ...
            },
            {
                updateTextStyle: {
                    range: {
                        startIndex: cleanContent.indexOf('3. Procompare Lead Generation') + 1,
                        endIndex: cleanContent.indexOf('3. Procompare Lead Generation') + '3. Procompare Lead Generation'.length + 1
                    },
                    textStyle: {
                        bold: true,
                        fontSize: { magnitude: 14, unit: 'PT' },
        ...
        console.log('✅ Document completely reformatted! All markdown removed and proper formatting applied');
        
    } catch (error) {
        console.error('❌ Error reformatting document:', error.message);
        console.error('Stack trace:', error.stack);
    }
}

completelyReformatDocument();
